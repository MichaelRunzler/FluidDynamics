...
public class PurifierScreen extends MachineScreenBase<PurifierContainer>
{
    ...
    protected void renderTooltip(@NotNull PoseStack stack, int x, int y) {
        ...
        renderFractionalTooltip(stack, x, y, 8, 24, 16, 68, menu.getFluidLevel(), menu.getMaxFluidLevel(), "mB");
    ...
    protected void renderBg(@NotNull PoseStack stack, float ticks, int x, int y)
    {
        ...
        int progressOverlay = Math.min(fpDivideMult(menu.getProgress().get(), menu.getMaxProgress().get(), 28), 28);
